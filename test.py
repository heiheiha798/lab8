...
def main():
    ...
    # --- 1. 生成矩阵 A (递增的有规律代码, SINT8) ---
    # 创建一个从 1 到 total_elements 的递增序列，然后重塑为矩阵
    # 临时使用 int16 避免在生成序列时溢出，再强制转换为 SINT8
    matrix_a_orig = np.arange(1, total_elements + 1, dtype=np.int16).reshape(MATRIX_DIM_TB, MATRIX_DIM_TB)
    matrix_a_orig = matrix_a_orig.astype(DATA_TYPE_IN) # 强制转换为 SINT8，这会导致大于127的值溢出并变为负数，但仍保持规律性

    # --- 2. 生成矩阵 B (递增的有规律代码, SINT8) ---
    # 与矩阵 A 使用相同的递增规律
    matrix_b_orig = np.arange(1, total_elements + 1, dtype=np.int16).reshape(MATRIX_DIM_TB, MATRIX_DIM_TB)
    matrix_b_orig = matrix_b_orig.astype(DATA_TYPE_IN) # 强制转换为 SINT8
