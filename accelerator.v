...
// A single unified module definition for both simulation and synthesis
module accelerator #(
    ...
    // NOTE: These ports are connected to SRAM instances for simulation
    // and become top-level ports for synthesis.
    // The port widths are defined using localparams inside the module body.
    ...
    output wire [TILE_SIZE * ACCUM_DATA_WIDTH-1:0]       sram_c_ping_wdata,
    output wire [$clog2(TILE_SIZE)-1:0]         sram_c_ping_raddr_A,
    input  wire [TILE_SIZE * ACCUM_DATA_WIDTH-1:0]       sram_c_ping_rdata_A,
    output wire [$clog2(TILE_SIZE * TILE_SIZE * ACCUM_DATA_WIDTH / MAIN_MEM_DATA_WIDTH_BITS)-1:0]  sram_c_ping_raddr_B,
    ...
    output wire [TILE_SIZE * ACCUM_DATA_WIDTH-1:0]       sram_c_pong_wdata,
    output wire [$clog2(TILE_SIZE)-1:0]         sram_c_pong_raddr_A,
    input  wire [TILE_SIZE * ACCUM_DATA_WIDTH-1:0]       sram_c_pong_rdata_A,
    output wire [$clog2(TILE_SIZE * TILE_SIZE * ACCUM_DATA_WIDTH / MAIN_MEM_DATA_WIDTH_BITS)-1:0]  sram_c_pong_raddr_B,
    ...
    // --- SRAMs ---
    // In synthesis, top-level ports connect to SRAMs.
    // In simulation, these wires connect to the instantiated SRAM modules below.
    wire [MAIN_MEM_DATA_WIDTH_BITS-1:0] sram_a_ping_wdata_internal, sram_a_pong_wdata_internal, sram_b_ping_wdata_internal, sram_b_pong_wdata_internal;
    wire [DF_SRAM_DATA_WIDTH-1:0] sram_a_ping_rdata_internal, sram_a_pong_rdata_internal, sram_b_ping_rdata_internal, sram_b_pong_rdata_internal;
    wire [SA_SRAM_C_ROW_WIDTH-1:0] sram_c_ping_wdata_internal, sram_c_pong_wdata_internal, sram_c_ping_rdata_A_internal, sram_c_pong_rdata_A_internal;
    wire [MAIN_MEM_DATA_WIDTH_BITS-1:0] sram_c_ping_rdata_B_internal, sram_c_pong_rdata_B_internal;

`ifndef SYNTHESIS
    // For simulation, instantiate the behavioral SRAM models
    // Connect external ports to these instances.
    assign sram_a_ping_wdata = sram_a_ping_wdata_internal;
    assign sram_a_pong_wdata = sram_a_pong_wdata_internal;
    assign sram_b_ping_wdata = sram_b_ping_wdata_internal;
    assign sram_b_pong_wdata = sram_b_pong_wdata_internal;
    assign sram_c_ping_wdata = sram_c_ping_wdata_internal;
    assign sram_c_pong_wdata = sram_c_pong_wdata_internal;

    assign sram_a_ping_rdata_internal = sram_a_ping_rdata;
    assign sram_a_pong_rdata_internal = sram_a_pong_rdata;
    assign sram_b_ping_rdata_internal = sram_b_ping_rdata;
    assign sram_b_pong_rdata_internal = sram_b_pong_rdata;
    assign sram_c_ping_rdata_A_internal = sram_c_ping_rdata_A;
    assign sram_c_pong_rdata_A_internal = sram_c_pong_rdata_A;
    assign sram_c_ping_rdata_B_internal = sram_c_ping_rdata_B;
    assign sram_c_pong_rdata_B_internal = sram_c_pong_rdata_B;
    
    sram_banked #(.IS_SRAM_A(1'b1), .NUM_BANKS(TILE_SIZE), .BANK_DEPTH(TILE_SIZE), .BANK_DATA_WIDTH(INPUT_DATA_WIDTH), .BUS_DATA_WIDTH(MAIN_MEM_DATA_WIDTH_BITS)) sram_a_ping (.clk(clk), .we(sram_a_ping_we), .waddr(sram_a_ping_waddr), .wdata(sram_a_ping_wdata_internal), .raddr(sram_a_ping_raddr), .rdata(sram_a_ping_rdata));
    sram_banked #(.IS_SRAM_A(1'b1), .NUM_BANKS(TILE_SIZE), .BANK_DEPTH(TILE_SIZE), .BANK_DATA_WIDTH(INPUT_DATA_WIDTH), .BUS_DATA_WIDTH(MAIN_MEM_DATA_WIDTH_BITS)) sram_a_pong (.clk(clk), .we(sram_a_pong_we), .waddr(sram_a_pong_waddr), .wdata(sram_a_pong_wdata_internal), .raddr(sram_a_pong_raddr), .rdata(sram_a_pong_rdata));
    sram_banked #(.IS_SRAM_A(1'b0), .NUM_BANKS(TILE_SIZE), .BANK_DEPTH(TILE_SIZE), .BANK_DATA_WIDTH(INPUT_DATA_WIDTH), .BUS_DATA_WIDTH(MAIN_MEM_DATA_WIDTH_BITS)) sram_b_ping (.clk(clk), .we(sram_b_ping_we), .waddr(sram_b_ping_waddr), .wdata(sram_b_ping_wdata_internal), .raddr(sram_b_ping_raddr), .rdata(sram_b_ping_rdata));
    sram_banked #(.IS_SRAM_A(1'b0), .NUM_BANKS(TILE_SIZE), .BANK_DEPTH(TILE_SIZE), .BANK_DATA_WIDTH(INPUT_DATA_WIDTH), .BUS_DATA_WIDTH(MAIN_MEM_DATA_WIDTH_BITS)) sram_b_pong (.clk(clk), .we(sram_b_pong_we), .waddr(sram_b_pong_waddr), .wdata(sram_b_pong_wdata_internal), .raddr(sram_b_pong_raddr), .rdata(sram_b_pong_rdata));
    sram_c_accum #(.NUM_ROWS(TILE_SIZE), .ELEM_PER_ROW(TILE_SIZE), .ELEM_WIDTH(ACCUM_DATA_WIDTH), .BUS_DATA_WIDTH(MAIN_MEM_DATA_WIDTH_BITS)) sram_c_ping (.clk(clk), .rst_n(rst_n), .we(sram_c_ping_we), .waddr(sram_c_ping_waddr), .wdata(sram_c_ping_wdata_internal), .raddr_A(sram_c_ping_raddr_A), .rdata_A(sram_c_ping_rdata_A), .raddr_B(sram_c_ping_raddr_B), .rdata_B(sram_c_ping_rdata_B));
    sram_c_accum #(.NUM_ROWS(TILE_SIZE), .ELEM_PER_ROW(TILE_SIZE), .ELEM_WIDTH(ACCUM_DATA_WIDTH), .BUS_DATA_WIDTH(MAIN_MEM_DATA_WIDTH_BITS)) sram_c_pong (.clk(clk), .rst_n(rst_n), .we(sram_c_pong_we), .waddr(sram_c_pong_waddr), .wdata(sram_c_pong_wdata_internal), .raddr_A(sram_c_pong_raddr_A), .rdata_A(sram_c_pong_rdata_A), .raddr_B(sram_c_pong_raddr_B), .rdata_B(sram_c_pong_rdata_B));
`endif
