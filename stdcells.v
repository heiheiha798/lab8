...
module DFFR_X1 (D, RN, CK, Q, QN); // <--- 这是我们新的行为级模型，名字是 DFFR_X1
  input D;    // Data input
  input RN;   // Reset Negative (active low asynchronous reset)
  input CK;   // Clock input
  output reg Q;  // Output
  output QN; // Inverted Output

  assign QN = ~Q; // QN is always the inverse of Q

  // Behavioral description
  // Sensitive to positive edge of clock or negative edge of reset
  always @(posedge CK or negedge RN) begin
    if (RN == 1'b0) begin // If reset is active (low)
      Q <= 1'b0;          // Output Q is reset to 0
    end else begin        // Else, on the positive edge of the clock
...
module DFFR_X1_UDP_ORIGINAL (D, RN, CK, Q, QN);
  input D;
  input RN;
  input CK;
  output Q;
